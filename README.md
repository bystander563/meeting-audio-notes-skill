...
- 可选 `pyannote.audio` 说话人分离，将内容标记为 `SPEAKER_00/01`。
...
的模型条款，并通过 `hf auth login` 保存只读 Token，或设置 `HF_TOKEN`。说话人分离只能区分不同声音，不能自动知道真实姓名。
...
- 说话人分离不能仅凭声音自动推断真实姓名，重叠发言和低质量电话音频也会降低准确率。
